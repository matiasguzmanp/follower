...
class Tracker():
    ...
    def __init__(self):
        ...
        self.base_angular_pid = PID(1, 0.1, 0.05, setpoint=320)
        self.base_distance_pid = PID(1, 0.1, 0.05, setpoint=self.following_distance)
        ...
        self.target_frame = "tulio/base_link"

        self.tf_buffer = tf2_ros.Buffer()
        self.tf_listener = tf2_ros.TransformListener(self.tf_buffer)
        self.transform = self.tf_buffer.lookup_transform(self.target_frame, "tulio/camera_depth_optical_frame", rospy.Time(0), rospy.Duration(1.0))
        ...
        self.cmd_vel_pub = rospy.Publisher("/tulio/cmd_vel", Twist, queue_size=10)
        ...
        rospy.Subscriber("/tulio/camera/depth/color/points", PointCloud2, self.realsense_image_callback, queue_size=1)
    ...
    def realsense_image_callback(self, msg):
        try:
            _points_data = rnp.numpify(msg)
            image_data = _points_data['rgb'].view(
            (np.uint8, 4))[..., [0, 1, 2]]
            _image_data = np.ascontiguousarray(image_data)
            _resolution = _image_data.shape[:2]

            if not _image_data is None:
                results = self.model(_image_data)


            twist = Twist()
            twist.linear.x = self.compute_distance_control(results, _points_data)
            twist.angular.z = self.compute_rotation_control(results)
    ...
    def compute_rotation_control(self, results):
            x1, y1, x2, y2 = results[0].boxes.xyxy.cpu().numpy()[0]
            resolution = results[0].orig_shape[:2]

            # Calculate the center of the bounding box
            x_center = (x1 + x2) // 2

            # Calculate the error to the middle of the image

            # Center the person w/r to the base
            control = self.base_angular_pid(x_center)

            ang_vel = 0.01*control
            

            if abs(ang_vel) > 0.6:
                return 0.6 if ang_vel > 0 else -0.6

            return ang_vel

    def compute_distance_control(self, results, points_data):
        x1, y1, x2, y2 = results[0].boxes.xyxy.cpu().numpy()[0]

        # Calculate the center of the bounding box
        x_center = (x1 + x2) // 2
        y_center = (y1 + y2) // 2
        z = points_data['z'][int(y_center),int(x_center)]

        print(f"DISTANCE: {z}")
        error = z - self.following_distance
        ...
        if abs(lin_vel) > 1.0:
            return 1.0 if lin_vel > 0 else 0.0
